...
namespace PrimaryConnect.Controllers
{
    ...
    public class SubmittedDocumentController : ControllerBase
    {
        ...
        //    // المجلد الثابت للمرفقات
        //    var folder = "SubmitedDocuments/Parent";
        ...
        //    // اكتشاف نوع المحتوى
        //    var mimeType = UsefulFunctions.GetMimeType(doc.FileName);

        //    // عرض الملف في المتصفح مباشرة
        //    return PhysicalFile(filePath, mimeType);
        //}
        [HttpGet("view/{id}")]
        public async Task<IActionResult> ViewDocument(int id)
        {
            var resource = await _context.documents.FindAsync(id);
            if (resource == null)
                return NotFound("document not found.");

            var path = Path.Combine(_env.WebRootPath, "SubmitedDocuments/Parent", resource.FileName);
